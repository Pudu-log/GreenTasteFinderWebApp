...
<body>
	...
	<div id="container">
		<div id="filerAndSerchBox">
			<span> <select id="sortSelect" onchange="sortRestaurants()">
					<option value="rating">별점높은</option>
					<option value="reviewCount" }>리뷰 많은</option>
					<option value="distance" }>가까운</option>
	...
	<div class="pagination">
		...
		<c:if test="${pagingBtn.prevBtn}">
			<a href="/?page=${currentPage - 1}&sortBy=${param.sortBy}&keyword=${param.keyword}">이전</a>
		</c:if>
		...
		<c:if test="${pagingBtn.nextBtn}">
			<a href="/?page=${currentPage + 1}&sortBy=${param.sortBy}&keyword=${param.keyword}">다음</a>
		</c:if>
	</div>
	</div>
