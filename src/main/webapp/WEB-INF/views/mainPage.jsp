...
<body>
	...
	<div id="container">
		<div id="info-box">
			<select id="sortSelect" class="custom-select"
				...
				<option value="rating" ${sortBy == 'rating' ? 'selected' : ''}>별점
					높은</option>
				<option value="reviewCount"
					${sortBy == 'reviewCount' ? 'selected' : ''}>리뷰 많은</option>
		...
		<div id="filerAndSerchBox">
			<span>
				<div id="tagContainer">
					<button class="tag" data-keyword="" onclick="onTagClick(this)">#home</button>
					<button class="tag" data-keyword="noodle"
						onclick="onTagClick(this)">#면요리</button>
					<button class="tag" data-keyword="chinese"
						onclick="onTagClick(this)">#중식</button>
					<button class="tag" data-keyword="japan" onclick="onTagClick(this)">#일식</button>
					<button class="tag" data-keyword="korean"
						onclick="onTagClick(this)">#한식</button>
					<button class="tag" data-keyword="fastfood"
						onclick="onTagClick(this)">#패스트푸드</button>
					<button class="tag" data-keyword="chicken"
						onclick="onTagClick(this)">#치킨</button>
					<button class="tag" data-keyword="salad" onclick="onTagClick(this)">#샐러드</button>
					<button class="tag" data-keyword="bakery"
						onclick="onTagClick(this)">#빵</button>
					<button class="tag" data-keyword="cafe" onclick="onTagClick(this)">#카페</button>
				</div>
				...
				<div class="restaurant" data-store-id="${restaurant.placeId}">
					...
					<div class="restaurant-content">
						...
						<div class="details">
							...
							<div>
								<span> <fmt:formatNumber value="${restaurant.distance}"
										type="number" maxFractionDigits="2" /> m
								</span>
							</div>
						</div>
						<!-- 음식점 세부정보 -->
	...
	<div class="pagination">
		...
		<!-- 페이지 번호 -->
		<c:forEach begin="${pagingBtn.startPage}" end="${pagingBtn.endPage}"
			var="i">
			<c:choose>
				...
				<c:otherwise>
					<a
						href="/?page=${i}&sortBy=${param.sortBy}&keyword=${param.keyword}">${i}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		<!-- 다음 버튼 -->
